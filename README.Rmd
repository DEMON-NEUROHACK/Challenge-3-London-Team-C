...
Next, we sought to assess whether the variant- and gene-level data modality were sufficiently different across individuals to be useful as classifier predictors. This is an indirect assessment approach in lieu of access to real participant-levle data (as opposed the dummy VCFs file we had access to). We computed pairwise Pearson correlations (r) across all individuals (repeated for each data modality separately). This demonstrated that most (though not all) data modalities had evidence of varying across individuals. 
