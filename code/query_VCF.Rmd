...
```{r}
all_ids <-  stringr::str_split(basename(sv_vcfs),"[.]",simplify = T)[,1]
...
sv_types <- unique(grl[[1]]$SVTYPE)
genotypes_list <- lapply(sv_types, function(x){
    mat <- lapply(grl,function(gr){
    # gr <- grl[[1]]
    gr <- subset(gr, SVTYPE==x)
    GenomicRanges::mcols(gr)$GT_int <- ifelse(gr$GT== "./.", 1, 0)
    ...
    as("sparseMatrix") %>%
    Matrix::t()
    data.table::fwrite(data.table::data.table(as.matrix(mat), keep.rownames = "id"),
                       paste0("data/SV_SNPs/by_variant/SV_VCFs.",gsub(":","-",x),".tsv.gz"), 
                       sep="\t")
    message(x," : ",paste(dim(mat),collapse = " x "))
    return(mat)
}) %>% `names<-`(sv_types)
# saveRDS(genotypes_list,file = "data/SV_SNPs/SV_VCFs.genotypes.rds")
