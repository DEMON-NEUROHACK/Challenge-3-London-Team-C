...
```{r}
gwas <- data.table::fread("raw_data/GWAS/ALS_sumstats_EUR_only.txt.gz")
...
```{r}
topss <- data.table::fread("raw_data/GWAS/tableS21.csv") %>% 
    data.table::merge.data.table(gwas %>% 
                                     subset(rsid %in% topss$SNP) %>%
                                     dplyr::mutate(SNP=rsid), 
                                 by="SNP")
data.table::fwrite(topss, "raw_data/GWAS/tableS21_coords.csv")
```
